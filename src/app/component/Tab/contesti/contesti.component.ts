...
import { AddContestiComponent } from "../../add-edit/add-edit-contesti/add-contesti.component";
...
@Component({
  ...
  templateUrl: "./contesti.component.html",
  styleUrls: ["./contesti.component.css"],
})
export class ContestiComponent implements OnInit {
  ...
  onClickEdit(p: PeriodicElement) {
    const dialogRef = this.dialog.open(AddContestiComponent, {
...
function EditAttivitaComponent(EditAttivitaComponent: any, arg1: {
  //  this.list.map((data) => data.codice
  // data:{this.list.map((data) => data.codice)}
  data: { p: PeriodicElement[]; };
}) {
